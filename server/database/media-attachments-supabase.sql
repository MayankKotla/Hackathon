-- Update media_attachments table to use Supabase Storage instead of Cloudinary
-- Drop existing table and recreate with new schema

DROP TABLE IF EXISTS media_attachments CASCADE;

-- Create media_attachments table for photos and videos using Supabase Storage
CREATE TABLE media_attachments (
  id SERIAL PRIMARY KEY,
  recipe_id INTEGER NOT NULL,
  user_id INTEGER NOT NULL,
  media_type VARCHAR(10) CHECK (media_type IN ('image', 'video')) NOT NULL,
  supabase_path VARCHAR(500) NOT NULL, -- Path in Supabase Storage
  supabase_url VARCHAR(500) NOT NULL,   -- Public URL from Supabase Storage
